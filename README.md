...
```bash
  uv run release_manager.py zip build/dist/Nextcloud_CLI nextcloud_cli 1.0.0 --build --upload
```
...
```bash
  uv run release_manager.py zip dist/Nextcloud_CLI nextcloud_cli 1.0.0 --include config --upload
```
...
```bash
  uv run release_manager.py files dist/Nextcloud_CLI nextcloud_cli 1.0.0 --upload
```